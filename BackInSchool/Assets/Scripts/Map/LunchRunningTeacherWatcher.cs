...
public class LunchRunningTeacherWatcher : MonoBehaviour
{
    ...
    [SerializeField] private float patrolMinMoveDistance = 0.45f;
    [SerializeField] private float patrolPauseMinSeconds = 0.35f;
    ...
    private float patrolTargetLocalX;
    ...
    private void TickPatrol()
    {
        ...
        float nextX = Mathf.MoveTowards(currentX, patrolTargetLocalX, Mathf.Max(0.05f, patrolSpeed) * Time.deltaTime);
    ...
    private void ResetPatrolState(bool snapToOrigin)
    {
        hasPatrolTarget = false;
        ...
        local.x = patrolOriginLocalPosition.x;
        transform.localPosition = local;
    }

    private float PickNextPatrolTargetX(float currentX)
    {
        float maxOffset = Mathf.Max(0.05f, patrolDistance);
        float minMove = Mathf.Clamp(patrolMinMoveDistance, 0f, maxOffset);
        float minX = patrolOriginLocalPosition.x - maxOffset;
        float maxX = patrolOriginLocalPosition.x + maxOffset;

        float chosen = currentX;
        for (int i = 0; i < 10; i++)
        {
            chosen = Random.Range(minX, maxX);
            if (Mathf.Abs(chosen - currentX) >= minMove)
                return chosen;
        }
